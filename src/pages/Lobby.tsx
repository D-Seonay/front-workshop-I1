...
const Lobby = () => {
  ...
  // ✅ Vérification du code et redirection si problème
  useEffect(() => {
    if (!room && !code) {
      navigate('/');
  ...
  useEffect(() => {
    if (room?.status === 'playing' && room.players.length === 2 && room.players.every(p => p.isReady && p.role)) {
      navigate('/paris');
    }
      ...
      <Card className="max-w-2xl w-full p-8 bg-card/80 backdrop-blur">
        <div className="text-center mb-8">
          ...
          <div className="flex items-center justify-center gap-3">
            ...
            <Button variant="ghost" size="sm" onClick={copySessionCode} className="gap-2">
            ...
            <div className="grid md:grid-cols-2 gap-4">
              ...
              >
                <div className="flex flex-col items-center text-center">
                  ...
                  {currentPlayer?.role === 'agent' && <Badge className="bg-primary">Sélectionné</Badge>}
                  {otherPlayer?.role === 'agent' && <Badge variant="secondary">Pris par l'autre joueur</Badge>}
                </div>
              </Card>
              ...
              >
                <div className="flex flex-col items-center text-center">
                  ...
                  {currentPlayer?.role === 'operator' && <Badge className="bg-secondary">Sélectionné</Badge>}
                  {otherPlayer?.role === 'operator' && <Badge variant="secondary">Pris par l'autre joueur</Badge>}
                </div>
              </Card>
            </div>
          </div>
